...
if __name__ == "__main__":
  ...
  import pickle
  """
  # looking if parsing has happened on this machine before
  if os.path.isfile("wav_files.pkl"):
    print("found wav_files.pkl")
    with open("wav_files.pkl", "rb") as fp:   # Unpickling
      wav_files = pickle.load(fp)
  else:
    wav_files = [] 
  
  if os.path.isfile("txt_files.pkl"):
    print("found txt_files.pkl")
    with open("txt_files.pkl", "rb") as fp:   # Unpickling
      txt_files = pickle.load(fp)
  else:
    txt_files = [] 
  """
  wav_files = []
  ...
  txt_source = '/content/data/assembly_ai_0/'
  for file_ in os.listdir(txt_source):
    if file_.endswith(".wav.txt"):
        txt_files.append(txt_source + file_)
    
  with open("txt_files.pkl", "wb") as fp:   #Pickling
      pickle.dump(txt_files, fp)

  asr_parse_to_json(txt_files, wav_files, wav_source, txt_source)
